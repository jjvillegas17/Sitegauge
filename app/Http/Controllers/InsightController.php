...
class InsightController extends BaseController{
	...
	public function getLikePeakDates($pageId){  // new_like/avg*100 > 300%
		$peakDates = \DB::select(\DB::raw("SELECT  new_likes, cast(new_likes/(select avg(new_likes) from page_metrics where facebook_page_id = :id0)*100 as unsigned) as `avg`, date_retrieved from page_metrics where facebook_page_id = :id and new_likes/(select avg(new_likes) from page_metrics where facebook_page_id = :id2)*100 >= 300 and date_retrieved >= (select min(created_time) from post_details_metrics where facebook_page_id = :id3) order by new_likes desc limit 5"), ['id0' => $pageId,'id' => $pageId, 'id2' => $pageId, 'id3' => $pageId]);

		// select new_likes, new_likes/(select avg(new_likes) from page_metrics)*100 as `avg`, date_retrieved from page_metrics where facebook_page_id = 341693319367148 and new_likes/(select avg(new_likes) from page_metrics)*100 >= 300 and date_retrieved >= (select min(created_time) from post_details_metrics) order by new_likes desc
